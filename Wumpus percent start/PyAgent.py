...
class Agent:
    ...
    def getpit(self):
        ...
        for i in self.breezes:
            ...
            if ((not [x+1,y] in self.visited) and (not [x+1,y] in self.tovisit) and (not [x+1,y] in self.pits) and x+1<=4):
                self.pits.append([x+1,y])
            if ((not [x-1,y] in self.visited) and (not [x-1,y] in self.tovisit) and (not [x-1,y] in self.pits) and x-1>=1):
                self.pits.append([x-1,y])
            if ((not [x,y+1] in self.visited) and (not [x,y+1] in self.tovisit) and (not [x,y+1] in self.pits) and y+1<=4):
                self.pits.append([x,y+1])
            if ((not [x,y-1] in self.visited) and (not [x,y-1] in self.tovisit) and (not [x,y-1] in self.pits) and y-1>=1):
                self.pits.append([x, y-1])
            i.append(x)
            i.append(y)
